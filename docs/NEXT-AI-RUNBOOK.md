...
現在の `tests/home-state.test.cjs` は旧app.js専用のVMモックで、5件すべて `HomeState is not defined` で失敗する。
...
- 8案それぞれのSVG、スマホサイズでの見え方、比較画像を用意。
- `icon-candidates.html` を作り、現在のホーム・設定からのリンク切れを解消する。
